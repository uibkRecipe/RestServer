...
public class HibernateManager {
	...
	@Produces(MediaType.APPLICATION_JSON)
	public List<IngredientType> findIngredientByName(@PathParam("ingredientName") String name){
	...
	@Path("/ingredient")
	@Consumes(MediaType.APPLICATION_JSON)
	public Response addIngredientToRecipe(int recipeID, List<IngredientType> ingredients,
			List<String> quantity){
		if (composedOfManager.addIngredientToRecipe(recipeID, ingredients, quantity)) {
			return Response.status(200).entity(true).build();
		}
		return Response.status(200).entity(false).build();
	}
